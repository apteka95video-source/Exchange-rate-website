...
Сайт без React, Vite, npm та build-інструментів. Для графіка використовується
Chart.js через CDN.

## Що показує сайт

Сайт працює як фінансовий дашборд:

- показує останні курси USD та EUR з Excel;
- показує актуальний курс USD та EUR з API НБУ на дату відкриття сайту;
- дозволяє фільтрувати Excel-курси за валютою та датами;
- будує графік динаміки USD та EUR;
- має історичний калькулятор конвертації UAH у валюту за курсом Excel;
- показує компактну таблицю останніх 10 рядків з можливістю розкрити всі рядки.
